...
describe('UsersController', () => {
  ...
  describe('findAll', () => {
    it('should find all users', async () => {
      await fixtures.user({ email: 'user1@test.com', name: 'User One' });
      await fixtures.user({ email: 'user2@test.com', name: 'User Two' });
  ...
  describe('update', () => {
    it('should update a user', async () => {
      const createdUser = await fixtures.user();

      const updatedUser = await controller.update(String(createdUser.id), {
        name: 'Jane Smith',
  ...
  describe('findAllPaginated', () => {
    it('should find all users paginated', async () => {
      for (let i = 0; i < 15; i++) {
        await fixtures.user({ email: `user${i}@test.com`, name: `User ${i}` });
      }
